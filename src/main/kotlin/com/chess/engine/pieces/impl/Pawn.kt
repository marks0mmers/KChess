...
import com.chess.engine.board.move.pawn.*
...
class Pawn(
    ...
    override fun movePiece(move: Move) =
        PieceUtils.getMovedPawn(move.movedPiece?.pieceAlliance, move.destinationCoordinate)
                    ...
                    currentCandidateOffset == 8 && board.getPiece(candidateDestinationCoordinate) == null -> {
                        ...
                        when {
                            pieceAlliance.isPawnPromotionSquare(candidateDestinationCoordinate) -> {
                                add(
                                    PawnPromotion(pawnMove,
                                    PieceUtils.getMovedQueen(pieceAlliance, candidateDestinationCoordinate)
                                )
                                )
                                add(
                                    PawnPromotion(pawnMove,
                                    PieceUtils.getMovedRook(pieceAlliance, candidateDestinationCoordinate)
                                )
                                )
                                add(
                                    PawnPromotion(pawnMove,
                                    PieceUtils.getMovedKnight(pieceAlliance, candidateDestinationCoordinate)
                                )
                                )
                                add(
                                    PawnPromotion(pawnMove,
                                    PieceUtils.getMovedBishop(pieceAlliance, candidateDestinationCoordinate)
                                )
                                )
                            }
                            else -> add(pawnMove)
